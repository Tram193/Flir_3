...
#define UART2_WRITE_BUF_SIZE     1024
#define UART3_READ_BUF_SIZE      8192
#define UART3_WRITE_BUF_SIZE     2048
#define UART5_READ_BUF_SIZE      1024
#define UART5_WRITE_BUF_SIZE     1024
#define UART6_READ_BUF_SIZE      256
#define UART6_WRITE_BUF_SIZE     256
...
#ifdef USING_UART_PORT_5
static T_RingBuffer s_uart5ReadRingBuffer;
static T_UartBufferState s_uart5ReadBufferState;
static T_RingBuffer s_uart5WriteRingBuffer;
static T_UartBufferState s_uart5WriteBufferState;
CCMRAM static uint8_t s_uart5ReadBuf[UART5_READ_BUF_SIZE];
CCMRAM static uint8_t s_uart5WriteBuf[UART5_WRITE_BUF_SIZE];

static T_DjiMutexHandle s_uart5Mutex;
static UART_HandleTypeDef s_uart5Handle;
#endif
...
CCMRAM static uint8_t s_uart6ReadBuf[UART6_READ_BUF_SIZE];
CCMRAM static uint8_t s_uart6WriteBuf[UART6_WRITE_BUF_SIZE];
...
#ifdef USING_UART_PORT_5
        case UART_NUM_5: {
            RingBuf_Init(&s_uart5ReadRingBuffer, s_uart5ReadBuf, UART5_READ_BUF_SIZE);
            RingBuf_Init(&s_uart5WriteRingBuffer, s_uart5WriteBuf, UART5_WRITE_BUF_SIZE);

            s_uart5Handle.Instance = UART5;
            s_uart5Handle.Init.BaudRate = baudRate;
            s_uart5Handle.Init.WordLength = UART_WORDLENGTH_8B;
            s_uart5Handle.Init.StopBits = UART_STOPBITS_1;
            s_uart5Handle.Init.Parity = UART_PARITY_NONE;
            s_uart5Handle.Init.HwFlowCtl = UART_HWCONTROL_NONE;
            s_uart5Handle.Init.Mode = UART_MODE_TX_RX;
            s_uart5Handle.Init.OverSampling = UART_OVERSAMPLING_16;
            HAL_UART_Init(&s_uart5Handle);
            __HAL_UART_ENABLE_IT(&s_uart5Handle, UART_IT_RXNE);

            Osal_MutexCreate(&s_uart5Mutex);
        }
            break;
#endif
...
#ifdef USING_UART_PORT_5
        case UART_NUM_5: {
            Osal_MutexLock(s_uart5Mutex);
            readRealSize = RingBuf_Get(&s_uart5ReadRingBuffer, buf, readSize);
            Osal_MutexUnlock(s_uart5Mutex);
        }
            break;
#endif
...
int UART_Write(E_UartNum uartNum, const uint8_t *buf, uint16_t writeSize)
{
    ...
    switch (uartNum) {
        ...
        case UART_NUM_5: {
            Osal_MutexLock(s_uart5Mutex);
            writeRealLen = RingBuf_Put(&s_uart5WriteRingBuffer, buf, writeSize);
            __HAL_UART_ENABLE_IT(&s_uart5Handle, UART_IT_TXE);
            usedCapacityOfBuffer = UART5_WRITE_BUF_SIZE - RingBuf_GetUnusedSize(&s_uart5WriteRingBuffer);
            s_uart5WriteBufferState.maxUsedCapacityOfBuffer =
                usedCapacityOfBuffer > s_uart5WriteBufferState.maxUsedCapacityOfBuffer ? usedCapacityOfBuffer
                                                                                       : s_uart5WriteBufferState.maxUsedCapacityOfBuffer;
            s_uart5WriteBufferState.countOfLostData += writeSize - writeRealLen;
            Osal_MutexUnlock(s_uart5Mutex);
        }
            break;
#endif
...
#ifdef USING_UART_PORT_5
        case UART_NUM_5:
            memcpy(readBufferState, &s_uart5ReadBufferState, sizeof(T_UartBufferState));
            memcpy(writeBufferState, &s_uart5WriteBufferState, sizeof(T_UartBufferState));
            break;
#endif
...
/**
 * @brief UART5 interrupt request handler fucntion.
 */
#ifdef USING_UART_PORT_5
...
void UART5_IRQHandler(void)
{
    ...
    if (__HAL_UART_GET_IT_SOURCE(&s_uart5Handle, UART_IT_RXNE) != RESET &&
        __HAL_UART_GET_FLAG(&s_uart5Handle, UART_FLAG_RXNE) != RESET) {
        data = (uint8_t) ((uint16_t) (s_uart5Handle.Instance->DR & (uint16_t) 0x01FF) & (uint16_t) 0x00FF);
        realCountPutBuffer = RingBuf_Put(&s_uart5ReadRingBuffer, &data, 1);
        usedCapacityOfBuffer = UART5_READ_BUF_SIZE - RingBuf_GetUnusedSize(&s_uart5ReadRingBuffer);
        s_uart5ReadBufferState.maxUsedCapacityOfBuffer =
            usedCapacityOfBuffer > s_uart5ReadBufferState.maxUsedCapacityOfBuffer ? usedCapacityOfBuffer
                                                                                  : s_uart5ReadBufferState.maxUsedCapacityOfBuffer;
        s_uart5ReadBufferState.countOfLostData += 1 - realCountPutBuffer;
    }

    if (__HAL_UART_GET_IT_SOURCE(&s_uart5Handle, UART_IT_TXE) != RESET &&
        __HAL_UART_GET_FLAG(&s_uart5Handle, UART_FLAG_TXE) != RESET) {
        if (RingBuf_Get(&s_uart5WriteRingBuffer, &data, 1)) {
            /* Transmit Data */
            s_uart5Handle.Instance->DR = ((uint16_t) data & (uint16_t) 0x01FF);
        } else {
            __HAL_UART_DISABLE_IT(&s_uart5Handle, UART_IT_TXE);
        }
    }
}
