...
/* Private constants ---------------------------------------------------------*/
//#define PPS_GPIO_CLK_ENABLE()   __HAL_RCC_GPIOD_CLK_ENABLE()
...
#define PPS_GPIO_CLK_ENABLE()   __HAL_RCC_GPIOA_CLK_ENABLE()
#define PPS_PORT                GPIOA
#define PPS_PIN                 GPIO_PIN_5
#define PPS_IRQn                EXTI9_5_IRQn
#define DjiTest_PpsIrqHandler   EXTI9_5_IRQHandler
#define PPS_IRQ_PRIO_PRE        0x0F
#define PPS_IRQ_PRIO_SUB        0x0F
...
void DjiTest_PpsIrqHandler(void)
{
    ...
    if (__HAL_GPIO_EXTI_GET_IT(PPS_PIN) != RESET) {
        ...
        psdkStat = osalHandler->GetTimeMs(&timeMs);
