...
void DjiUser_StartTask(void const *argument)
{
    ...
    T_DjiHalNetworkHandler halNetworkHandler = {
        .NetworkInit = SomComm_netWorkInit,
        .NetworkDeInit = SomComm_netWorkDeInit,
        .NetworkGetDeviceInfo = SomComm_networkGetDeviceInfo,
    };
...
//Attention: if you want to run payload sdk on extension port, please define the macro USE_USB_HOST_UART.
    ...
    returnCode = DjiPlatform_RegHalNetworkHandler(&halNetworkHandler);
    if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
...
//    }
    ...
    returnCode = SomComm_Init();
    if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        USER_LOG_ERROR("start som comm error");
    }
