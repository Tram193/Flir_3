...
#define GREMSY_GIMBAL_ENABLE                1

#define SMOOTH_FACTOR_MAX                   30
#define SMOOTH_FACTOR_MIN                   0

#define SMOOTH_TARGET_MAX                   1000
#define SMOOTH_TARGET_MIN                   1

/* Pre-calcualte the floating point stuff as far as possible at compile time*/
#define SMOOTH_SCALE_FACTOR ((SMOOTH_TARGET_MAX - SMOOTH_TARGET_MIN) / (SMOOTH_FACTOR_MAX - SMOOTH_FACTOR_MIN))
#define SMOOTH_SCALE_OFFSET (float)(SMOOTH_TARGET_MAX - (SMOOTH_SCALE_FACTOR * SMOOTH_FACTOR_MAX + 0.5))
...
#if(GREMSY_GIMBAL_ENABLE == 1)
        djiStat = GremsyGMB_Init();
        if (djiStat != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
            USER_LOG_ERROR("Gremsy Gimbal init error: 0x%08llX", djiStat);
        }
#endif
                                     ...
                                     T_DjiAttitude3d rotationValue)
{
    ...
    switch (rotationMode) {
        ...
        case DJI_GIMBAL_ROTATION_MODE_ABSOLUTE_ANGLE:
            ...
            GremsyGMB_SetRotation(rotationValue.pitch, rotationValue.roll, rotationValue.yaw, DJI_GIMBAL_ROTATION_MODE_ABSOLUTE_ANGLE);
        ...
        case DJI_GIMBAL_ROTATION_MODE_SPEED:
            ...
            /*!< Test gimbal set speed*/
            GremsyGMB_SetSpeed(rotationValue.pitch, rotationValue.roll, rotationValue.yaw);
...
//    }
    ...
    while (1) {
        ...
        if(USER_UTIL_IS_WORK_TURN(step, 2, PAYLOAD_GIMBAL_TASK_FREQ))
        {
            /*!< Send heartbeat to gimbal with Freq 2Hz*/
            GimbalGremsy_sendHeartbeattoGimbal();
        }
        
        if(GremsyGMB_IsPresent() == true && firstInitialization == true)
        {
            /*!< Get gimbal param*/
            GimbalGremsy_ParameterProcess();
        }
        else if(GremsyGMB_IsPresent() == true && firstInitialization == false) {
            /*!< Reset gimbal */
//            GremsyGMB_SetReboot();
            
            /*!< Set gimbal initialization */
            firstInitialization = true;
            
            /*!< Wait for gimbal initialization */
        ...
        if (USER_UTIL_IS_WORK_TURN(step, 50, PAYLOAD_GIMBAL_TASK_FREQ)) {
            ...
            GremsyGMB_SendAttitude(&s_aircraftAttitude, timestamp.millisecond);
        }
        
        if (USER_UTIL_IS_WORK_TURN(step, 5, PAYLOAD_GIMBAL_TASK_FREQ)) {
...
//                PsdkLogger_UserLogInfo("Optical: %f, Digital: %f, factor: %f", opticalZoomFactor, digitalZoomFactor, (1 / (opticalZoomFactor * digitalZoomFactor)));
                
//                if(opticalZoomFactor >= 10.0f) {
//                    opticalZoomFactor = opticalZoomFactor*1.5f;
//                }
                
                djiStat = GremsyGMB_SetSpeedConversionFactor(1 / (opticalZoomFactor * digitalZoomFactor));
                if (djiStat != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
...
static T_DjiReturnCode GetSystemState(T_DjiGimbalSystemState *systemState)
{
    ...
    GremsyGMB_GetPitchSpeed((uint8_t*)&GremsyGMB->systemState.maxSpeedPercentage.pitch);
    GremsyGMB_GetPitchsmooth((uint8_t*)&GremsyGMB->systemState.smoothFactor.pitch);
    GremsyGMB_GetYawSpeed((uint8_t*)&GremsyGMB->systemState.maxSpeedPercentage.yaw);
    GremsyGMB_GetYawsmooth((uint8_t*)&GremsyGMB->systemState.smoothFactor.yaw);

    /*!< Get system status from the gimbal */
    *systemState = GremsyGMB->systemState;
...
static T_DjiReturnCode GetAttitudeInformation(T_DjiGimbalAttitudeInformation *attitudeInformation)
{
    ...
    /*!< TODO: Using callback function set attitude function*/
    s_attitudeInformation.attitude.pitch = GremsyGMB->attitudeInformation.attitude.pitch;
    s_attitudeInformation.attitude.roll = GremsyGMB->attitudeInformation.attitude.roll;
    s_attitudeInformation.attitude.yaw = GremsyGMB->attitudeInformation.attitude.yaw;
...
static T_DjiReturnCode GetRotationSpeed(T_DjiAttitude3d *rotationSpeed)
{
    ...
    /*!< Get gimbal rotation speed*/
    s_speed = GremsyGMB->rotationSpeed;
...
static T_DjiReturnCode SetControllerSmoothFactor(uint8_t smoothingFactor, E_DjiGimbalAxis axis)
{
    ...
    /*!< Convert smoothing Factor to Gremsy smooth*/
    uint16_t smoothConverted = smoothingFactor*SMOOTH_SCALE_FACTOR + SMOOTH_SCALE_OFFSET;
    
    if (axis == DJI_GIMBAL_AXIS_PITCH) {
        s_systemState.smoothFactor.pitch = smoothingFactor;
        
        /*!< Set gimbal speed for pitch axis*/
        GremsyGMB_SetPitchSmooth(s_systemState.smoothFactor.pitch);
    }
    else if (axis == DJI_GIMBAL_AXIS_YAW) {
        s_systemState.smoothFactor.yaw = smoothingFactor;
        /*!< Set gimbal speed for yaw axis*/
        GremsyGMB_SetYawSmooth(s_systemState.smoothFactor.yaw);
    }
    else
...
static T_DjiReturnCode SetControllerMaxSpeedPercentage(uint8_t maxSpeedPercentage, E_DjiGimbalAxis axis)
{
    ...
    if (axis == DJI_GIMBAL_AXIS_PITCH) {
        ...
        /*!< Set gimbal speed for pitch axis*/
        GremsyGMB_SetPitchSpeed(s_systemState.maxSpeedPercentage.pitch);
    }
    else if (axis == DJI_GIMBAL_AXIS_YAW) {
        s_systemState.maxSpeedPercentage.yaw = maxSpeedPercentage;
    
        /*!< Set gimbal speed for pitch axis*/
        GremsyGMB_SetYawSpeed(s_systemState.maxSpeedPercentage.yaw);
    }
    else
        USER_LOG_ERROR("axis is not supported.");
...
static T_DjiReturnCode RestoreFactorySettings(void)
{
    ...
    /*!< Set param default to gimbal*/
    GremsyGMB_SetPitchSpeed(s_systemState.maxSpeedPercentage.pitch);
    GremsyGMB_SetYawSpeed(s_systemState.maxSpeedPercentage.yaw);
    
    GremsyGMB_SetFreeMode();
    GremsyGMB_SetPitchSmooth(s_systemState.smoothFactor.pitch);
    GremsyGMB_SetYawSmooth(s_systemState.smoothFactor.yaw);
    
    if(GremsyGMB->info.type == 0x0A) {
        GremsyGMB_SetPitchStiffness(40);
        GremsyGMB_SetRollStiffness(35);
        GremsyGMB_SetYawStiffness(50);
        
        GremsyGMB_SetPitchHoldStrength(30);
        GremsyGMB_SetRollHoldStrength(30);
        GremsyGMB_SetYawHoldStrength(30);
        
        GremsyGMB_SetGyroFilter(2);
        GremsyGMB_SetOutputFilter(4);
    }
    else if(GremsyGMB->info.type == 0x4A) {
        GremsyGMB_SetPitchStiffness(100);
        GremsyGMB_SetRollStiffness(120);
        GremsyGMB_SetYawStiffness(110);
        
        GremsyGMB_SetPitchHoldStrength(45);
        GremsyGMB_SetRollHoldStrength(45);
        GremsyGMB_SetYawHoldStrength(50);
    
        GremsyGMB_SetGyroFilter(4);
        GremsyGMB_SetOutputFilter(3);
    }
    else  if(GremsyGMB->info.type == 0x0C) {
...
static T_DjiReturnCode SetMode(E_DjiGimbalMode mode)
{
    ...
    /*!< Set gimbal mode */
    GremsyGMB->systemState.gimbalMode = s_systemState.gimbalMode;
    
    if(mode == DJI_GIMBAL_MODE_FREE) {
        GremsyGMB_SetFreeMode();
    }
    else if(mode == DJI_GIMBAL_MODE_YAW_FOLLOW) {
...
static T_DjiReturnCode Reset(E_DjiGimbalResetMode mode)
{
    ...
    switch (mode) {
        case DJI_GIMBAL_RESET_MODE_YAW:
            ...
            /*!< Set angle to return home position*/
            anglePanHome = 0;
        ...
        case DJI_GIMBAL_RESET_MODE_PITCH_DOWNWARD_UPWARD:
            ...
            /*!< Set gimbal angle here for return right position*/
            anglePanHome = GremsyGMB->currentYaw;
    ...
    /*!< Reset gimbal to a position */
    GremsyGMB_SetResetMode(mode);
